...
<!--                            </div>-->
        ...
        <!-- 分页 -->
        <Pagination v-bind:child-msg="pageparm" :page-sizes="pageparams.pageSizes" :layout="pageparams.layout" @callFather="callFather" v-if="showPagination"></Pagination>
        <!-- 弹窗 -->
                ...
                <el-row :gutter="formOptions.gutter?formOptions.gutter:20">
                    <el-col :span="fitem.form.span?fitem.form.span:24" v-for="(fitem,findex) in formColumns">
                        <el-form-item :label="fitem.label+'：'" :prop="fitem.prop" v-if="(fitem.form.hidden!==undefined && dialogTitle!=='详情')?!fitem.form.hidden:true">
                            ...
                            <ly-area v-else-if="fitem.type=='area'" :disabled="isFormItemEditDisabled(fitem.form.editDisabled)" v-model="formData[fitem.prop]"></ly-area>
                        </el-form-item>
                    </el-col>
                </el-row>
                <!-- 自定义form部分 -->
